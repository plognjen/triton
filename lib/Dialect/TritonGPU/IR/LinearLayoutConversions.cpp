...
LinearLayout chooseScaledMfmaScaleLayout(
  ...
  for (int32_t elem = 16; elem < tilePerWarpNonK * 16; elem *= 2)
    registerBase.emplace_back(std::vector<int32_t>{0, elem});

  llvm::outs() << "opIdx " << dotOperandIdx << "\n";
  LinearLayout tileLayout(
  ...
  if(dotOperandIdx == 1){
    auto tmp = warpsPerCTANew[0];
    warpsPerCTANew[0] = warpsPerCTANew[1];
    warpsPerCTANew[1] = tmp;

    tmp = warpOrder[0];
    warpOrder[0] = warpOrder[1];
    warpOrder[1] = tmp;
  }
  LinearLayout warpLayout = identityStandardND(kWarp, warpsPerCTANew, warpOrder);
  LinearLayout ctaLayout = tileLayout.transposeOuts(outDimNames) *
                           warpLayout.transposeOuts(outDimNames);
  auto ctaLay = CTALayoutAttr::get(/*context=*/ctx, /*CTAsPerCGA=*/{1, 1},
                                   /*CTASplitNum=*/{1, 1}, /*CTAOrder=*/{1, 0});
  auto finalLayout = combineCtaCgaWithShape(ctaLayout, ctaLay, dotOperandShape);
