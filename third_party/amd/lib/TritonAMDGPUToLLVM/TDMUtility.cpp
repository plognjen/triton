...
std::pair<SmallVector<unsigned>, unsigned> getPartitionAlignedWarpDistribution(
  ...
  // For 2D with partitionDim == innerDim, all non-inner dims were already
  // candidates above.  If warps remain, try the partition dim again
  // (only useful when partitionDim != innerDim and the first subdivision
  // pass couldn't absorb them due to alignment).
  if (partitionDim != innerDim) {
    while (remainingWarps >= 2 &&
           static_cast<int64_t>(warps[partitionDim] * 2) <=
               blockShape[partitionDim]) {
      warps[partitionDim] *= 2;
      remainingWarps /= 2;
    }
  }
...
void emitTDMLoadStore(
    RewriterBase &rewriter, Location loc,
    const LLVMTypeConverter *typeConverter, ArrayRef<Value> desc,
    ArrayRef<int64_t> blockShape, int numWarps, unsigned padInterval,
    unsigned padAmount, ArrayRef<Value> offset, ArrayRef<Value> dstPtrs,
    Value pred, Value multicastMask, Type elementType, Value barrierPtr,
    bool isLoad, const triton::LinearLayout &sharedLayout, Value ctaId,
    bool isRowMajor,
    std::optional<PartitionedSharedEncodingAttr> partitionedEnc) {
  auto b = TritonLLVMOpBuilder(loc, rewriter);
  ...
  triton::LinearLayout sliceLayout = [&] {
    if (triton::gpu::isPaddedEncoding(sliceEncoding)) {
      auto dummyTy = triton::gpu::MemDescType::get(
          sliceShape, IntegerType::get(ctx, 8), sliceEncoding,
          triton::gpu::SharedMemorySpaceAttr::get(ctx));
      return triton::gpu::paddedLinearLayout(dummyTy);
    }
    return triton::gpu::toLinearLayout(sliceShape, sliceEncoding);
  }();
