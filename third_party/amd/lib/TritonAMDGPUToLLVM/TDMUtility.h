...
//
// Supports 1D-5D tensors.  When partitionedEnc is set, the warp distribution
// is adjusted so each wave's tile fits within a single LDS partition.  If
// the warps cannot cover all logical pieces in one instruction, the
// operation is automatically split into multiple sequential TDM instructions.
//
...
// - sharedLayout: full shared memory LinearLayout (used for partition
// selection)
// - partitionedEnc: optional; constrains warp distribution to partition
//   boundaries and provides encoding info for multi-instruction splits
