...
def issue_loads_msplit(producer, a_desc0, a_desc1, b_desc, a_buffer0, a_buffer1, b_buffer,
    ...
    if not TRANSPOSE_B:
        ttgl.amd.gfx1250.tdm.async_load(b_desc, [producer * BLOCK_K, 0], b_buffer.index(buf_idx))
    else:
        ttgl.amd.gfx1250.tdm.async_load(b_desc, [0, producer * BLOCK_K], b_buffer.index(buf_idx))
    return producer + 1
...
def ws_pipe0_f16(a_desc0, a_desc1, b_desc, a_buffer0, a_buffer1, b_buffer,
    ...
    ttgl.barrier(cta=False, addrspace=0b00001)
    ttgl.sched_barrier(0)
    ...
    for _ in range(0, loop_ub):
        ...
        ttgl.sched_barrier(0)
        ttgl.amd.gfx1250.tdm.async_wait((NUM_BUFFERS - 2) * NUM_LOADS)
        ttgl.barrier(cta=True, addrspace=0b00001)
        ttgl.sched_barrier(0)
    ...
    offs_cm = pid_m * BLOCK_M + ttgl.arange(0, HALF_M, layout=ttgl.SliceLayout(1, WMMA_LAYOUT))
    offs_cn = pid_n * BLOCK_N + ttgl.arange(0, BLOCK_N, layout=ttgl.SliceLayout(0, WMMA_LAYOUT))
    offs_c = stride_cm * offs_cm[:, None] + stride_cn * offs_cn[None, :]
    mask_c = (offs_cm[:, None] < M) & (offs_cn[None, :] < N)
    ttgl.store(c_ptr + offs_c, accumulator, mask=mask_c)
...
def ws_pipe1_f16(a_buffer1, b_buffer,
    ...
    ttgl.assume(loop_ub >= 0)
    for _ in range(0, loop_ub):
        consumer, a, b = lds_load(consumer, a_buffer1, OPERAND_LAYOUT_A, b_buffer, OPERAND_LAYOUT_B, NUM_BUFFERS,
                                  TRANSPOSE_B)
        ttgl.sched_barrier(0)
        ttgl.barrier(cta=True, addrspace=0b00000)
        ttgl.sched_barrier(0)
    ...
    offs_cm = pid_m * BLOCK_M + HALF_M + ttgl.arange(0, HALF_M, layout=ttgl.SliceLayout(1, WMMA_LAYOUT))
    offs_cn = pid_n * BLOCK_N + ttgl.arange(0, BLOCK_N, layout=ttgl.SliceLayout(0, WMMA_LAYOUT))
    offs_c = stride_cm * offs_cm[:, None] + stride_cn * offs_cn[None, :]
    mask_c = (offs_cm[:, None] < M) & (offs_cn[None, :] < N)
    ttgl.store(c_ptr + offs_c, accumulator, mask=mask_c)
...
@pytest.mark.parametrize("M,N,K", [(2048, 2048, 2048)])
@pytest.mark.parametrize("DUMP", [False])
def test_runtime_gemm_tdm_warp_pipelined_ws(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, M, N, K, DUMP):
    if triton.cdiv(K, BLOCK_K) < NUM_BUFFERS:
        pytest.skip("Skip tests where K/BLOCK_K < NUM_BUFFERS")
    ...
    # 4 kernel warps (default partition) + 4 worker warps = 8 total
    num_warps = 4
    WARP_BASES = [(0, 1), (1, 0)]

    warp_bases = tuple(WARP_BASES)
    grid = (triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1)
    kernel = gemm_tdm_pipelined_warp_pipelined_ws_kernel[grid](
        a_device, b_device, c_device,  #
        M, N, K,  #
        stride_am, stride_ak,  #
        stride_bk, stride_bn,  #
        ...
        WARP_BASES=warp_bases, NUM_WARPS=num_warps,  #
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ws", action="store_true", help="Use warp-specialized variant")
